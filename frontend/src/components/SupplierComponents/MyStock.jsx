...
const MyStock = () => {
  ...
  // Update API URLs to use API_BASE_URL
  ...
  const fetchSalesStatistics = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_BASE_URL}/api/supplier-products/sales/statistics`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchSalesHistory = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_BASE_URL}/api/supplier-sales`, {
        headers: { Authorization: `Bearer ${token}` },
        params: {
  ...
  // Fetch Receipts with enhanced sorting and error handling
  const fetchReceipts = async () => {
    try {
      setReceiptsLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_BASE_URL}/api/supplier-receipts`, {
        headers: { Authorization: `Bearer ${token}` },
        params: {
  ...
  // Check if sale is already in any receipt
  ...
  // Handle sale selection with validation
  ...
  // Handle select all sales (only non-receipted ones)
  ...
  // Create Receipt function with pre-validation
  const createReceipt = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/api/supplier-receipts`, receiptData, {
        headers: { 
  ...
  // Refresh sales and receipts data
  ...
  const handleManualStockUpdate = async (productId, newQuantity) => {
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/api/supplier-products/${productId}`,
  ...
  const createSale = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/api/supplier-sales`, saleData, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const MobileFilters = () => (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-3 mb-3 border border-gray-200 dark:border-gray-700">
      <div className="flex justify-between items-center mb-2">
        <h3 className="text-xs font-semibold text-gray-900 dark:text-white">Filters</h3>
        <button
      ...
      <div className="space-y-2">
        ...
        <label className="flex items-center gap-2 px-2 py-1.5 bg-gray-100 dark:bg-gray-700 rounded cursor-pointer text-[11px] hover:bg-gray-200 dark:hover:bg-gray-600">
          <input
            type="checkbox"
  ...
  const renderStatisticsCards = () => (
    <div className={`grid grid-cols-2 ${isMobile ? 'gap-1' : 'gap-2'} mb-3`}>
  ...
  // Product Row - Enhanced for mobile
  const renderProductRow = (product) => {
    ...
    if (isMobile) {
      return (
        <div key={product._id} className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-3 mb-2">
          <div className="flex justify-between items-start mb-2">
            <div className="flex-1">
              <h3 className="font-semibold text-gray-900 dark:text-white text-sm mb-1">{product.name}</h3>
              <p className="text-gray-500 dark:text-gray-400 text-xs">{product.sku || 'No SKU'}</p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
            ...
            <div>
              ...
              <p className="font-semibold text-gray-900 dark:text-white">${product.sellingPrice?.toFixed(2)}</p>
            </div>
            <div>
              <p className="text-gray-500 dark:text-gray-400">Cost</p>
              <p className="text-gray-900 dark:text-white">${product.productionPrice?.toFixed(2)}</p>
            </div>
            <div>
          ...
          <div className="flex justify-between items-center">
            <span className={`inline-flex px-2 py-1 rounded text-xs font-semibold ${getStockStatusColor(stockStatus)}`}>
              {getStockStatusText(stockStatus)}
            </span>
  ...
  // Sale Card Component with receipt status
  ...
  // Receipts Tab Component with Cards
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-2 flex flex-col">
      ...
      {isMobile && activeTab === 'stock' && (
        <button
          onClick={() => setShowMobileFilters(!showMobileFilters)}
          className="flex items-center gap-1 px-3 py-2 bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 mb-3 w-full justify-center text-[11px] font-medium"
        >
